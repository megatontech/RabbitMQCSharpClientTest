...
{
    public class AutoMQClient : IRabbitMqClient
    {
        ...
        public static IRabbitMqClient Instance
        {
            get
            {
                if (_instanceClient.IsNull())
                    _instanceClient = RabbitMqClientFactory.CreateRabbitMqClientInstance(typeof(AutoMQClient));
        ...
        public void TriggerEventMessage(EventMessage eventMessage, string exChange, string queue)
        {
            ...
            using (Context.SendConnection)
            {
                ...
                {
                    Context.SendChannel.QueueDeclare(queue: "AUTO", durable: true, exclusive: false, autoDelete: false, arguments: null);
        ...
        {
            Context.ListenConnection = this.Context.SendConnection;//获取连接
